...
extension LocationManager {
  ...
  public static let live: LocationManager = { () -> LocationManager in
    ...
    manager.destroy = { id in
      .fireAndForget {
        dependencies[id]?.subscriber.onCompleted()
        dependencies[id] = nil
      }
    }
...
private struct Dependencies {
  ...
  let manager: CLLocationManager
  let subscriber: AnyObserver<LocationManager.Action>
}
...
private class LocationManagerDelegate: NSObject, CLLocationManagerDelegate {
  ...
  init(_ subscriber: AnyObserver<LocationManager.Action>) {
  ...
  func locationManager(
    ...
    subscriber.onNext(.didChangeAuthorization(status))
  }

  func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
    subscriber.onNext(.didFailWithError(LocationManager.Error(error)))
  }

  func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
    subscriber.onNext(.didUpdateLocations(locations.map(Location.init(rawValue:))))
  }
    ...
    func locationManager(
      ...
      subscriber.onNext(.didFinishDeferredUpdatesWithError(error.map(LocationManager.Error.init)))
    }
  #endif
    ...
    func locationManagerDidResumeLocationUpdates(_ manager: CLLocationManager) {
      subscriber.onNext(.didResumeLocationUpdates)
    }
  #endif

  #if os(iOS) || os(watchOS) || targetEnvironment(macCatalyst)
    func locationManager(_ manager: CLLocationManager, didUpdateHeading newHeading: CLHeading) {
      subscriber.onNext(.didUpdateHeading(newHeading: Heading(rawValue: newHeading)))
    }
  #endif

  #if os(iOS) || os(macOS) || targetEnvironment(macCatalyst)
    func locationManager(_ manager: CLLocationManager, didEnterRegion region: CLRegion) {
      subscriber.onNext(.didEnterRegion(Region(rawValue: region)))
    }
  #endif

  #if os(iOS) || os(macOS) || targetEnvironment(macCatalyst)
    func locationManager(_ manager: CLLocationManager, didExitRegion region: CLRegion) {
      subscriber.onNext(.didExitRegion(Region(rawValue: region)))
    }
  #endif
    ...
    func locationManager(
      ...
      subscriber.onNext(.didDetermineState(state, region: Region(rawValue: region)))
    }
  #endif
    ...
    func locationManager(_ manager: CLLocationManager, didStartMonitoringFor region: CLRegion) {
      subscriber.onNext(.didStartMonitoring(region: Region(rawValue: region)))
    }
  #endif
    ...
    func locationManager(_ manager: CLLocationManager, didVisit visit: CLVisit) {
      subscriber.onNext(.didVisit(Visit(visit: visit)))
    }
  #endif
}
